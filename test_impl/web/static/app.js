...
const LIST_SHIPMENT_COL_WEIGHTS = [
  1.2, 1.15, 1.15, 1.05, 1.35, 2.1, 0.85, 0.85, 0.85,
];
...
function applyListColFilterPanel() {
  ...
  const { allValues, selected } = listColFilterPending;
...
async function switchSubmodule(key) {
  ...
  const isCustomerInfo = key === "customerInfo";
  const isAi = key === "ai";
  const isReconcile = key === "reconcile";
  const isList = !isEntry && !isDelivery && !isCustomerInfo && !isAi && !isReconcile;
  ...
  document.getElementById("submoduleCustomerInfo")?.classList.toggle("is-hidden", !isCustomerInfo);
  ...
  document.querySelectorAll("[data-submodule]").forEach((el) => {
    ...
    if (el.classList.contains("sidebar-module-head-link")) {
  ...
  } else if (isCustomerInfo && typeof loadCustomerInfoAdmin === "function") {
    await loadCustomerInfoAdmin();
  } else if (isAi && typeof window.focusAiAssistant === "function") {
    window.focusAiAssistant();
  }
  if (typeof window.collapseSidebarForNavigation === "function") {
    window.collapseSidebarForNavigation();
  } else if (typeof window.updateSidebarOrdersActiveState === "function") {
