...
| BOM 查询 | BF-0006、BF-0015、BF-0021、BF-0022、BF-0024 |
| BOM 录入 | BF-0016、BF-0017、BF-0018 |
| Agent 治理 | BF-0009 |
| Git 推送 | BF-0010 |
| OCR 识别 | BF-0011 |
| 工序库存 | BF-0012、BF-0019、BF-0020、BF-0023 |
...
|------|------|
| 关联 CL | CL-0282 |
| 模块 | 库存录入（`inventory/service.py`、`inventory_entry.js`） |
| 现象 | 在途 8088 改 50 变成 8138；改 0 有时无法保存；换供应商才可保存 |
| 根因 | 看板显示在途**合计**，保存却写入**所选供应商空桶**（误加）；未改字段也一并提交 |
| 修复 | 默认选有库存供应商、按供应商填在途；仅提交变更桶；后端空桶+有合计时按工序合计校正 |
| 防复发 | `test_set_outsource_wrong_supplier_*`、`test_set_inhouse_only_*` |
| 验证 | 8088→50 合计为 50；仅改场内为 0 时在途不变 |
...
|------|------|
| 关联 CL | CL-0275 |
| 模块 | 供应商信息维护（`supplier-admin.js`、`app.js`） |
| 现象 | 列筛选后页面跳到最底部；切换模块再返回仍保留筛选状态 |
| 根因 | 列表行数减少后浏览器保持原 scrollY；子模块切换未清空 `createListColFilter`；表单类名变更导致首卡 `flex:1` 撑满、表格被挤到页面底部 |
| 修复 | 筛选变更时滚回表格区；`switchSubmodule('supplier')` 调用 `loadSupplierAdmin(true)` 清空筛选 |
| 防复发 | 手动：筛选→确认视口；切模块→返回见全量列表 |
| 验证 | 筛选 1 条后不在页脚；返回模块显示「共 N 条」非「显示 1/N」 |
...
|------|------|
| 关联 CL | CL-0267 |
| 模块 | BOM 联想搜索 / 主数据列表 / 批量导入（`cost_store`、`line_store`、`record_service`） |
| 现象 | 按品名搜「810」时 BOM 有 7 条相关记录，下拉只显示 5 条；同料号不同品名可能被导入覆盖误删 |
| 根因 | 去重/覆盖键仅为料号，未区分整串料号下的不同品名（如 `11*000000/04016-04` 的 810S头壳 vs 改良810STK2头壳） |
| 修复 | 联想与 `list_master_parts` 按 (整串料号, 客户, 品名) 去重；导入覆盖用 `find_import_overwrite_ids` 按品名定位 |
| 防复发 | 上述 3 个单元测试 + `normalize_part_no` 注释强调 `/` 后缀不可忽略 |
| 验证 | `CostStore.search_part_numbers(q='810')` 返回 7 条 |
...
| 关联 CL | CL-0263 |
| 模块 | 库存页（`inventory.js`） |
| 现象 | 编辑后点「返回总览」只显示刚编辑的一个料号 |
| 根因 | 进入编辑时写入搜索框的料号/品名/客户在返回时仍作为查询条件 |
| 修复 | 进入编辑前快照总览筛选，返回时还原后再查询 |
| 防复发 | 返回总览后搜索框应恢复进入编辑前的值 |
| 验证 | 多料号总览 → 编辑 → 返回，列表条数与进入前一致 |
...
|------|------|
| 关联 CL | CL-0259 |
| 模块 | `hover_tip.js`、`cost_query.js` |
| 现象 | 复制料号/品名时出现浮动复制框，遮挡操作 |
| 根因 | HoverTip 在 mouseenter/拖选时弹出 |
| 修复 | 选中/按下鼠标时隐藏；列表单元格不再绑定 HoverTip |
| 防复发 | 拖选复制时不应出现 `#hoverTip` |
| 验证 | BOM 查询列表 Ctrl+C 复制无浮层 |
...
|------|------|
| 关联 CL | CL-0258 |
| 模块 | BOM 查询预览/列表（`cost_common.js`、`cost_query.js`） |
| 现象 | 预览中多家供应商挤一行看不清；鼠标拖选复制品名/料号时自动打开预览 |
| 根因 | 供应商单行只读 input；表格行 click 未忽略文本选中 |
| 修复 | 多供应商标签逐条展示；选中/拖拽时不触发行 click |
| 防复发 | 手动拖选列表单元格不应弹窗 |
| 验证 | 预览 CNC 两行供应商；列表复制不误开 |
...
|------|------|
| 关联 CL | CL-0257 |
| 模块 | `inventory_bom_lookup.js` |
| 现象 | 库存搜索载入后，料号/品名/客户输入框下又出现一行相同信息的下拉 |
| 根因 | 程序填值触发 `input` 重开联想；下拉内嵌筛选框与选项重复展示当前值 |
| 修复 | 静默填值 + 载入后 `closeAllCombos`；`simpleList` 不再内嵌重复搜索框 |
| 防复发 | 载入工序库存后三框下方无浮层 |
| 验证 | TST-PL-001 载入后无重复行 |
...
|------|------|
| 关联 CL | CL-0250 |
| 模块 | 工序库存 `inventory/service.py`、`inventory_entry.js` |
| 现象 | BOM 查询修改工序/供应商后，工序库存搜索载入仍显示旧工序卡与旧供应商 |
| 根因 | 工序路由从 BOM 实时读取，但 API/前端未传 `customer_name`，同料号多客户或改条件后仍用旧缓存看板 |
| 修复 | `get_route(part, customer_name=…)`；`/api/inventory/route` 与 `loadAll` 带客户参数；输入变更时 `invalidateLoadedBoard`；**CL-0251**：卡片展示 BOM 全部供应商、下拉选料自动载入、客户模糊匹配 BOM |
| 防复发 | `test_inventory.py` 中 BOM 更新与客户隔离用例 |
| 验证 | 改 BOM → 工序库存同客户再载入；30 项 inventory 测试通过 |
