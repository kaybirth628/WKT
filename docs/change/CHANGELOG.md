...
| 字段 | 内容 |
|------|------|
| 涉及模块 | `inventory/service.py`、`inventory_entry.js` |
| 变更内容 | 库存编辑回归简单模型：各桶填多少存多少；外发在途仅改**所选供应商**桶，去掉合计归并逻辑 |
| 根因 | CL-0282 聚合校正与用户「改什么变什么」心智不符 |
| 防复发 | `test_set_outsource_per_supplier_target` |
| 验证 | 选供应商 A 改 50→A=50；选 B 改 30→B=30，互不影响 |
| SOP 同步 | 否 |
| SOP 免同步原因 | 库存校正简化 |
| 关联 | CL-0282 |

### CL-0282 · 2026-07-30 · 修复（C）

| 字段 | 内容 |
|------|------|
| 涉及模块 | `inventory/service.py`、`inventory_entry.js` |
| 变更内容 | 修复在途库存编辑：合计显示 vs 分供应商保存不一致导致误加/无法归零；仅提交变更数量 |
| 根因 | 见 BF-0027 |
| 防复发 | 见 BF-0027 |
| 验证 | 8088→50 结果为 50；只改场内时途不变 |
| SOP 同步 | 否 |
| SOP 免同步原因 | Bug 修复 |
| 关联 | BF-0027 |

### CL-0281 · 2026-07-30 · 修复（C）

| 字段 | 内容 |
|------|------|
| 涉及模块 | `wkt_events.py`、`notify-feishu-deploy.py`、`audit_log.js` |
| 变更内容 | 部署通知/操作记录仅推送**相对上次 top_cl 的增量**；摘要简化为 `CL-0279→CL-0280`；飞书与摘要内容一致 |
| 根因 | 每次取 CHANGELOG 头部 8 条，未对比上次已部署 CL |
| 防复发 | `test_deploy_delta_since_cl` |
| 验证 | 0279→0280 推送后飞书/操作记录仅列 CL-0280 一条 |
| SOP 同步 | 否 |
| SOP 免同步原因 | 运维通知 |
| 关联 | CL-0280 |
...
|------|------|
| 涉及模块 | `wkt_events.py` |
| 变更内容 | 飞书部署通知：解析 CHANGELOG 表格「变更内容」；快照改存 `data/` 保证 build 从→到；消息列出 `[类型] CL-xxxx：具体修复` |
| 根因 | 解析只认 `- 变更内容：`；快照在 deploy-info 易被覆盖导致无上一 build |
| 防复发 | `test_parse_changelog_head`（表格格式）、`test_format_deploy_transition` |
| 验证 | 再推送后飞书见 `版本/build 从→到` + 每条 CL 具体说明 |
| SOP 同步 | 否 |
| SOP 免同步原因 | 运维通知 |
| 关联 | CL-0277 |
...
| 字段 | 内容 |
|------|------|
| 涉及模块 | `wkt_events.py`、`audit_log.js`、`auth.css` |
| 变更内容 | 部署操作记录摘要含 CL 变更预览；悬停可看完整迭代列表；摘要列统一居中 |
| 根因 | — |
| 防复发 | `test_build_deploy_audit_summary` |
| 验证 | 操作记录「系统部署」行摘要可见 CL 条目；悬停显示完整 changes；摘要列居中 |
| SOP 同步 | 否 |
| SOP 免同步原因 | UI/摘要展示 |
| 关联 | CL-0277 |
